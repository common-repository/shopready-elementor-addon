...
class Elementor_Helper
{
    ...
    public static function get_global_setting($setting_id, $default = '')
    {
        ...
        if (!isset($woo_ready_el_global_settings['kit_settings'])) {
            ...
            $woo_ready_el_global_settings['kit_settings'] = method_exists($kit, 'get_settings') ? $kit->get_settings() : null;
        }

        if (!empty($woo_ready_el_global_settings['kit_settings'][$setting_id] ?? '')) {
