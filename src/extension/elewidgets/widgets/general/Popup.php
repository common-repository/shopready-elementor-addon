...
class Popup extends \Shop_Ready\extension\elewidgets\Widget_Base
{
	...
	protected function html()
	{
		...
		$style = sanitize_text_field($settings['_style'] ?? '');
		$this->add_render_attribute(
			'wrapper_style',
			[
				'class' => ['woo-ready-cart-popup-layout', esc_attr($style)],
			]
		);

		echo wp_kses_post(sprintf("<div %s>", $this->get_render_attribute_string('wrapper_style')));

		if (file_exists(dirname(__FILE__) . '/template-parts/popup/' . $style . '.php')) {

			shop_ready_widget_template_part(
				'general/template-parts/popup/' . $style . '.php',
				array(
					'settings' => $settings,
				)
			);
